...
std::vector <double> estimateHumanComfortableSafetyIndices(geometry_msgs::Pose2D cur_rpos, gazebo_msgs::ModelStates msg){
    ...
    if(min_orientationth == 999999) indices.push_back(1); else indices.push_back(min_orientationth);
...
void modelStatesCallback(const gazebo_msgs::ModelStatesConstPtr &msg){
    ...
    direction_ind_pub.publish(direction_index); pub_sdi_tdh.publish(sdi_tdh); pub_sdi_tdl.publish(sdi_tdl);//pub_sdi_tdh.publish(sdi_td_)
    ...
    fprintf(resultFile,"%f %f %f %f %f %f %f %f %f %f %f %f %f %f \n",cur_rpos_.x,cur_rpos_.y,cur_rpos_.theta,vrl,vra,collision_index.data,direction_index.data,interaction_index.data,motion_index1.data,motion_index.data,motion_index2.data,motion_index3.data,motion_index4.data,sdi_td_tmp_.data); //Write validity value to file
    fclose(resultFile);
}
//
/* */
...
int main(int argc, char ** argv)
{
    ...
    pub_sdi_tdh = nh.advertise<std_msgs::Float32>("/Tdh",5);
