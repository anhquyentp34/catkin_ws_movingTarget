...
ros::Publisher collision_ind_pub, direction_ind_pub, interaction_ind_pub, motion_ind_pub;
...
geometry_msgs::Pose2D selectApproachingHumans(gazebo_msgs::ModelStates human_states, std::vector <geometry_msgs::Pose2D> vhgroup){
    ...
    if(!vhgroup.empty())
    {
        shgroup = vhgroup[0];
        for(int i=0; i<human_states.name.size(); i++){
            p.x = human_states.pose[i].position.x;
            p.y = human_states.pose[i].position.y;
            if((p.x==shgroup.x)&(p.y==shgroup.y)){ // if robot approaches a person then change the radius
...
visualization_msgs::Marker createMarkerHumanBody(float_t x, float_t y, float_t z, int id_point, int id_zone){
    ...
    //
    marker.header.frame_id = "base_footprint";//map
    marker.header.stamp = ros::Time();
    marker.ns = ss.str();
    marker.id = id_point; // this number should be different
...
void detectedHumanCallback(const gazebo_msgs::ModelStates::ConstPtr &msg){
    ...
    visualization_msgs::MarkerArray approaching_marker, hmarker_array, hdmarker_array;
    ...
    std_msgs::Float32 collision_index, direction_index,interaction_index, robot_velocity, motion_index, motion_index1, motion_index2, motion_index3, motion_index4, motion_index5;
    ...
    direction_ind_pub.publish(direction_index);
    ...
    for(int i =0; i<human_states.name.size(); i++){
       ...
       // sitting people
        if(human_states.twist[i].angular.x){// = 1 sitting otherwise
            ...
            person.velocity.z = 0;
            if(vp<0.2){// standing
                person.velocity.x = 0.0*cos(cur_ptheta+M_PI_2);// vp
                person.velocity.y = 0.0*sin(cur_ptheta+M_PI_2);
            }else{ // moving
        ...
        hdmarker = createVector(person.position.x,person.position.y,person.position.z,0.5*cos(cur_ptheta),0.5*sin(cur_ptheta),i,"Human_direction",4);
