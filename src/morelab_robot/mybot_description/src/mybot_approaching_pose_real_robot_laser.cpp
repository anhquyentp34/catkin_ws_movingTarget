...
FILE * resultFile  = fopen("real_robot_results_laser.csv","w");
                                                                         ...
                                                                         geometry_msgs::Twist tw)
{
    ...
    if(vp<0.12) cur_ptheta = M_PI;
    else cur_ptheta = cur_anglev;

    //if(!fixed_pose) cur_ptheta = M_PI;
    cur_ptheta = M_PI;
...
geometry_msgs::Pose2D predictApproachingPoseOfMovingHumans(geometry_msgs::Pose2D pose)
{
    ...
    double x0 = pose.x, y0 = pose.y, theta0 =M_PI;//M_PI

    double dx = x-x0, dy = y-y0;
    double dis_app = sqrt(dx*dx+dy*dy);
    ROS_INFO("Predict approaching distance: %f", dis_app);
    bool flag_moving = true;
    if(flag_moving){
        if(moving_app_flag){ // if approaching pose is moving
            if((dis_app>1.6)&&(!fixed_pose)){//1.2

                app_pose.x = app_pose.x - 1.2; // predict
...
void publishHumanComfortableSafetyIndices(){
    ...
    resultFile = fopen("real_robot_results_laser.csv","a");
...
void humanInformationExtraction(){
    ...
    for(int i =0; i<human_states.name.size(); i++){
        ...
        geometry_msgs::Vector3Stamped inv, outv;
        inp.header.frame_id = "map";// base_footprint
        inp.point.x = human_states.pose[i].position.x;
        inp.point.y = human_states.pose[i].position.y;
        inp.point.z = 0;
        try{
            pListener->transformPoint("map", inp, outp);
        }
        catch(tf::TransformException& ex){
            ROS_ERROR("Received an exception trying to transform a point from \"base_footprint\" to \"map\": %s", ex.what());
        }
        //
        inv.header.frame_id = "map"; //base_footprint
        inv.vector.x = human_states.twist[i].linear.x;
        inv.vector.y = human_states.twist[i].linear.y;
        inv.vector.z = 0;
        try{
            pListener->transformVector("map", inv, outv);
        }
        catch(tf::TransformException& ex){
            ROS_ERROR("Received an exception trying to transform a point from \"base_footprint\" to \"map\": %s", ex.what());
        }
        double vpx = outv.vector.x;  double vpy = outv.vector.y;
        // end tf
        */
        double cur_ptheta;
        ...
        //person.position.x = outp.point.x; person.position.y = outp.point.y;
       ...
       // sitting people
        if(human_states.twist[i].angular.x){// = 1 sitting otherwise
            ...
            if(vp<hvel_threshold){// standing
                if(laser_data){
                    cur_ptheta = M_PI;
                    person.velocity.x = 0.12*cos(cur_ptheta+M_PI_2);// laser data
                    person.velocity.y = 0.12*sin(cur_ptheta+M_PI_2);// laser data
                    moving_app_flag = false;
                }else{
                ...
                if(vp>0.5) vp=0.5;
                person.velocity.x = 2.4*vp*cos(cur_ptheta+M_PI_2);// vp 3.0
                person.velocity.y = 2.4*vp*sin(cur_ptheta+M_PI_2);
                moving_app_flag = true;
            }
        }
        ROS_INFO("Velocity %f, %f *************************************",person.velocity.x,person.velocity.y);
...
void approachingPoseEstimation(){
    ...
    //
    if((approaching_flag)&&(shgroup.theta!=0)&&(!fixed_pose)){
            hgroup = shgroup;
...
void localCostmapCallback(const nav_msgs::OccupancyGridConstPtr & local_costmap)
{
    ...
    //}
    //ROS_INFO("####################### %s",local_costmap->header.frame_id.c_str());
}
//
...
void robotOdomCallback(const nav_msgs::Odometry::ConstPtr & msg){
    ...
    // tf odom transform to map
    geometry_msgs::PoseStamped inpose, outpose;
    inpose.header.frame_id = "odom";//
    inpose.pose.orientation = robot_odom_.pose.pose.orientation;
    inpose.pose.position = robot_odom_.pose.pose.position;
    try{
        pListener->transformPose("map", inpose, outpose);
    }
    catch(tf::TransformException& ex){
        ROS_ERROR("Received an exception trying to transform a point from \"base_footprint\" to \"map\": %s", ex.what());
    }
    cur_rpos_.x = outpose.pose.position.x; cur_rpos_.y = outpose.pose.position.y;
    cur_rpos_.theta = convertQuaternionToAngle(outpose.pose.orientation);
    // end tf
