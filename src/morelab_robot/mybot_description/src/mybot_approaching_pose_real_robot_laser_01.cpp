...
geometry_msgs::Pose2D predictApproachingPoseOfMovingHumans(geometry_msgs::Pose2D pose)
{
    ...
    bool flag_stop_pose = false;
    if(flag_moving){
        if(!flag_stop_pose){//1.2
            app_pose.x = app_pose.x - 1.8; // predict
       ...
       // sitting people
        if(human_states.twist[i].angular.x){// = 1 sitting otherwise
            ...
            if(vp<hvel_threshold){// standing
                ...
                person.velocity.x = 2*vp*cos(cur_ptheta+M_PI_2);// vp
                person.velocity.y = 2*vp*sin(cur_ptheta+M_PI_2);
            }
        }
