...
ros::Publisher collision_ind_pub, pub_sii_tp, pub_sii_tc,  direction_ind_pub, pub_sdi_tdh,pub_sdi_tdl, interaction_ind_pub,pub_sgi_tg, motion_ind_pub;
...
//
std_msgs::Float32 sdi_td_, sdi_td_tmp_;// using for the threshold of the social direction index
...
//
double computeOrientationIndex(double x, double y, double theta, double x0, double y0, double theta0,int8_t ind, std::string human_name){

    double orientation = 999999;
    //for(int i = 0; i<human_states.name.size(); i++){
        if((ind==0)||(ind==1)){
            if((human_name == "p0")||(human_name == "p1")){
                orientation = orientationIndex(x,y,theta,x0,y0,theta0);
            }
        }
        if((ind==6)||(ind==7)){
            if((human_name == "p2")||(human_name == "p3")||(human_name == "p4")){
                orientation = orientationIndex(x,y,theta,x0,y0,theta0);
            }
        }
        if((ind==3)||(ind==4)){
            if((human_name == "p5")||(human_name == "p6")||(human_name == "p7")){
                orientation = orientationIndex(x,y,theta,x0,y0,theta0);
            }
        }
        if((ind==9)||(ind==10)){
        ...
        if((ind==13)||(ind==14)){
            if((human_name == "p15")||(human_name == "p16")){
                orientation = orientationIndex(x,y,theta,x0,y0,theta0);
            }
        }
        if((ind==15)||(ind==16)){
        ...
        if((ind==22)||(ind==23)){
            if((human_name == "p10")||(human_name == "p11")){
                orientation = orientationIndex(x,y,theta,x0,y0,theta0);
            }
        }
        if((ind==25)||(ind==26)){
            if((human_name == "p12")||(human_name == "p13")){
                orientation = orientationIndex(x,y,theta,x0,y0,theta0);
            }
        }
        if((ind==28)||(ind==29)){
...
std::vector <double> estimateHumanComfortableSafetyIndices(geometry_msgs::Pose2D cur_rpos, gazebo_msgs::ModelStates msg){
    ...
    double min_orientation = 999999; double min_orientationth = 999999; double max_motion = 0; double max_motion1 = 0;
    ...
    for(int i=0; i<msg.name.size(); i++){
        ...
        double tmp1 = 999999; double tmp1th = 999999;
        /*
        if((ind==1)||(ind==2)){
        ...
        */
        tmp1 = computeOrientationIndex(x,y,theta,x0,y0,theta0,ind,msg.name[i]);
        if(tmp1<min_orientation){
            min_orientation = tmp1;
        }
        //
        tmp1th = orientationIndex(app_pose_.x,app_pose_.y,app_pose_.theta,x0,y0,theta0);
        if(tmp1th<min_orientationth){
            min_orientationth = tmp1th;
        }
    ...
    for(int i=0;i<vhgroup_.size();i++){
        ...
        // calculate interaction index
        double tmp3 = interactionIndex(x,y,x0,y0,1,rg/2.2,rg/2.2,0);
        if(tmp3>max_interaction){
            max_interaction = tmp3;
        }
    }
    //
    if(min_orientationth == 999999) min_orientationth=1;
    if(min_orientation>min_orientationth) min_orientation = min_orientationth-0.025;
    //
    if(max_collision>0.14) max_collision = 0.14;
    //
    indices.push_back(max_collision);
    indices.push_back(max_interaction);
    indices.push_back(min_orientation);
    indices.push_back(min_orientationth);
        ...
        if(ind==5){
            if((human_states.name[i] == "p2")||(human_states.name[i] == "p3")||(human_states.name[i] == "p4")){
                ...
                if(k1==3){
                    ...
                    sdi_td_.data = sdi_td_tmp_.data+0.0;
                }
            }
        }
        if(ind==2){
...
void modelStatesCallback(const gazebo_msgs::ModelStatesConstPtr &msg){
    ...
    std_msgs::Float32 collision_index,sii_tp, sii_tc, direction_index, sdi_tdl, sdi_tdh, interaction_index, sgi_tg;
    std_msgs::Float32 robot_velocity, motion_index, motion_index1, motion_index2, motion_index3, motion_index4, motion_index5;
    std_msgs::Float32 aconstant; aconstant.data=0.1;

    std::vector<double> indices= estimateHumanComfortableSafetyIndices(cur_rpos_, human_states);
    collision_index.data = indices[0];  interaction_index.data = indices[1];
    direction_index.data = indices[2] ;   sdi_td_tmp_.data = indices[3]; sdi_tdl.data = 0.67; sdi_tdh.data = 0.835;
    motion_index.data = indices[4];motion_index1.data = indices[5];motion_index2.data = indices[6];motion_index3.data = indices[7];motion_index4.data = indices[8];motion_index5.data = indices[9];
    ...
    direction_ind_pub.publish(direction_index); pub_sdi_tdh.publish(sdi_td_); //pub_sdi_tdh.publish(sdi_tdh); pub_sdi_tdl.publish(sdi_tdl);
    ...
    fprintf(resultFile,"%f %f %f %f %f %f %f %f %f %f %f %f %f %f \n",cur_rpos_.x,cur_rpos_.y,cur_rpos_.theta,vrl,vra,collision_index.data,direction_index.data,interaction_index.data,motion_index1.data,motion_index.data,motion_index2.data,motion_index3.data,sdi_td_.data,sdi_td_tmp_.data); //Write validity value to file
    fclose(resultFile);
}
//
/* */
...
int main(int argc, char ** argv)
{
    ...
    pub_sdi_tdh = nh.advertise<std_msgs::Float32>("/Td",5);
    //pub_sdi_tdl = nh.advertise<std_msgs::Float32>("/Tdl",5);
    ...
    //
    sdi_td_.data = 1.0; sdi_td_tmp_.data = 1.0;
    //
    //ros::Rate rate(30);
    ros::Rate rate(20);
    while(ros::ok())
